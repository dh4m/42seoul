...
/*   Updated: 2023/03/05 14:34:58 by dham             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float	cone_distance(t_ray *ray, t_obj *obj)
{
	...
	const float	cos_square = \
		square_f(obj->height) / (square_f(obj->height) + square_f(obj->diameter / 2));
	float		det_c[3];
	t_vec		ce;
...
t_vec	cone_nomal_v(t_vec *hit_p, t_obj *hit_obj)
{
	...
	temp = vec_multi(&hit_obj->nomal_v, vec_square(&cp) / vec_inner(&hit_obj->nomal_v, &cp));
	n = vec_minus(&cp, &temp);
	vector_normalize(&n);
	return (n);
}